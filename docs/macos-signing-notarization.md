...
The compiled macOS `zz` binaries shipped on GitHub Releases are signed with a
...
submission (see [#776](https://github.com/Fred-Ko/zz/issues/776)).
...
- Source-checkout installs (`bun run setup`) do not set a quarantine bit, so
...
Drop these into a working directory (default `~/zz-signing`):
...
```sh
scripts/ci-macos-upload-secrets.sh ~/zz-signing --dry-run   # validate first
scripts/ci-macos-upload-secrets.sh ~/zz-signing             # upload all five
...
APPLE_API_KEY_ID=… APPLE_API_ISSUER_ID=… APPLE_API_KEY=… \
  bash scripts/ci-macos-sign.sh packages/coding-agent/binaries/zz-darwin-arm64
```
