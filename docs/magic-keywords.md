...
Magic keywords are standalone words in a user prompt that add a hidden instruction for that turn. They are enabled by default and glow in the editor when `zz` recognizes them.
...
zz config set magicKeywords.enabled false

# Disable one keyword while leaving the others enabled
zz config set magicKeywords.ultrathink false
zz config set magicKeywords.orchestrate false
zz config set magicKeywords.workflow false
```

All four settings default to `true`. Run `zz config list` to inspect every available setting and its current value. See [Settings](./settings.md) for configuration scopes, precedence, and project-local overrides.
