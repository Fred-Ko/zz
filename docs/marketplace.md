...
A **marketplace** is a Git repository (or local directory) containing a catalog file at `.zz-plugin/marketplace.json` (preferred) or `.claude-plugin/marketplace.json` (Claude Code-compatible fallback). The catalog lists available plugins with their sources, descriptions, and metadata.

A **plugin** is a directory containing Claude/ZZ plugin content such as skills, commands, agents, hooks, tools, MCP servers, or LSP servers. Marketplace installs also load extension modules declared by `package.json#zz.extensions`; legacy manifest aliases remain readable for compatibility. Installation symlinks the cached plugin into the scope's `node_modules` tree and records it in `zz-plugins.lock.json`, the same runtime surfaces used by npm-installed and `zz plugin link`ed plugins. Plugins are identified by `name@marketplace` (e.g. `code-review@claude-plugins-official`).

**Scopes**: marketplace plugins can be installed at two scopes:

- **user** (default) -- available in all projects, stored in `~/.zz/plugins/installed_plugins.json`
- **project** -- available only in the active project, stored in the nearest project `.zz/plugins/installed_plugins.json`
...
```
zz plugin marketplace add <source>
zz plugin marketplace remove <name>
zz plugin marketplace update [name]
zz plugin marketplace list
zz plugin discover [marketplace]
zz plugin install [--force] [--scope user|project] name@marketplace
zz plugin uninstall [--scope user|project] name@marketplace
zz plugin upgrade [--scope user|project] [name@marketplace]
zz plugin enable [--scope user|project] name@marketplace
zz plugin disable [--scope user|project] name@marketplace
```
...
Git and local sources must contain a catalog at `.zz-plugin/marketplace.json` (preferred) or `.claude-plugin/marketplace.json` (Claude Code-compatible fallback). Direct catalog URLs cache only the JSON catalog; plugins in URL-sourced catalogs cannot use relative string sources like `"./plugins/foo"`.

## Catalog format (marketplace.json)

A marketplace catalog lives at `.zz-plugin/marketplace.json` in the repository root. When ZZ is the only intended consumer, prefer this path. To remain Claude Code-compatible (ZZ loads the same shape from either path), publish at `.claude-plugin/marketplace.json` instead — ZZ uses it as a fallback when `.zz-plugin/marketplace.json` is absent. A repository may ship both: ZZ reads the `.zz-plugin/` copy, Claude Code reads the `.claude-plugin/` copy. Same catalog format either way:
...
~/.zz/
  ...
  plugins/
    ...
    zz-plugins.lock.json          # Runtime enable/feature state
...
<project>/.zz/
  plugins/
    ...
    zz-plugins.lock.json          # Project runtime enable/feature state
    node_modules/<package>        # Symlink to the cached plugin
```
