...
A marketplace catalog lives at `.omp-plugin/marketplace.json` in the repository root. When zz is the only intended consumer, prefer this path. To remain Claude Code-compatible (zz loads the same shape from either path), publish at `.claude-plugin/marketplace.json` instead — zz uses it as a fallback when `.omp-plugin/marketplace.json` is absent. A repository may ship both: zz reads the `.omp-plugin/` copy, Claude Code reads the `.claude-plugin/` copy. Same catalog format either way:
