...
```text
Config sources (.zz/.claude/.cursor/.vscode/mcp.json, mcp.json, etc.)
...
In practice MCP servers also come from higher-priority providers (for example native `.zz/...` and tool-specific config dirs). Authoring guidance:

- Prefer `.zz/mcp.json` (project) or `~/.zz/agent/mcp.json` (user) for explicit control.
...
ZZ-native MCP config (`.zz/mcp.json`, `~/.zz/agent/mcp.json`, plus their `.mcp.json` variants) expands `${VAR}` and `${VAR:-default}` placeholders recursively before converting to runtime config. It also accepts boolean/string forms for `enabled` (`true`, `false`, `1`, `0`) and numeric strings for `timeout`.
