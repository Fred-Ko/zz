...
2. Check legacy compiled-binary location:
   - Windows: `%LOCALAPPDATA%/zz` (fallback `%USERPROFILE%/AppData/Local/zz`)
   - non-Windows: `~/.local/bin`
3. Fall back to packaged `native/` and executable directory candidates.

`getNativesDir()` uses `$XDG_DATA_HOME/zz/natives` when `$XDG_DATA_HOME/zz` exists; otherwise it uses `~/.zz/natives`.
