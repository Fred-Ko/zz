...
Single source of truth for cache wiring, emitted as a bazelrc fragment (its `rc` output) that consumers pass via `bazelisk --bazelrc=...` (or `ZZ_BAZEL_RC` for the driver). Two modes are selected via `BAZEL_REMOTE_USER`/`BAZEL_REMOTE_PASSWORD` (injected from the `bazel-remote-ci` secret on kata pods only):
...
| GitHub-hosted | `--config=ci --disk_cache=~/.cache/zz-bazel-disk --repository_cache=~/.cache/zz-bazel-repo` |
...
`.github/actions/bazel-natives` is the direct builder: `bazel-cache` → `ZZ_BAZEL_RC=<rc> bun scripts/bazel-natives.ts <targets> --dest <dest>`, followed by a disk-cache save after a hosted miss. `.github/actions/native-artifacts` is the no-build consumer: download `native-addons` → run the same driver with `--source`. Every TypeScript test job requests `linux-x64-baseline linux-x64-modern` through one of these actions.
...
3. Runtime candidate order includes:
   ...
   - versioned cache dir,
   - legacy compiled-binary dir (`%LOCALAPPDATA%/zz` on Windows, `~/.local/bin` elsewhere),
...
- Root: `/data/cache/pi-natives` (provisioned by `entrypoint.sh` alongside the cargo caches, owned `root:zz`, mode `02770` setgid so cached files inherit `gid=zz` and stay readable by every slot user).
...
| `ROBOMP_NATIVES_CACHE_ROOT`                 | `/data/cache/pi-natives` | Cache root directory. Must be `root:zz 02770` for cross-slot reads.                                |
