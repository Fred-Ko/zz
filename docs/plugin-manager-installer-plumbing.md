...
This document describes how `zz plugin` npm/git/link and marketplace operations mutate plugin state on disk and become runtime capabilities. Marketplace installs keep their own registries and cache, then register the cached plugin through the same `node_modules` and `zz-plugins.lock.json` runtime surfaces used by npm/git/link installs; see `docs/marketplace.md`.
...
zz plugin install name@marketplace / zz install name@marketplace
  -> MarketplaceManager
  -> mutate marketplace registries and cache
  -> symlink the cached package into the scope's node_modules and update zz-plugins.lock.json
...
Global plugin state lives under `~/.zz/plugins`:
...
- `zz-plugins.lock.json` — runtime state for npm/link/marketplace plugins:
...
- `~/.zz/marketplaces.json` — configured marketplace catalogs
- `~/.zz/plugins/installed_plugins.json` — user-scoped marketplace installs
- `<cwd>/.zz/plugins/installed_plugins.json` — project-scoped marketplace installs when available
- `~/.zz/plugins/cache/{marketplaces,plugins}/` — cached catalogs and plugin directories
- `<scope>/plugins/node_modules/<package>` — symlink to the cached plugin, allowing its canonical `package.json#zz.extensions` and tools to load; legacy manifest aliases remain readable for compatibility
- `<scope>/plugins/zz-plugins.lock.json` — enablement and feature state shared with the runtime plugin loader
...
- A package missing `zz`/`pi` is still installable and listable.
- Runtime plugin loading (`getEnabledPlugins`) skips packages without `zz`/`pi` manifest.
...
- `zz plugin install pkg@newVersion` updates dependency and lockfile version.
...
1. Read plugin dependency map from `~/.zz/plugins/package.json`.
2. Load lockfile runtime config (missing file -> empty defaults).
3. Load project overrides (`<cwd>/.zz/plugin-overrides.json`, parse/read errors -> empty object with warning).
...
`link` supports local plugin development by symlinking a local package into `~/.zz/plugins/node_modules/<pkg.name>`.
