...
```bash
zz --mode rpc [regular CLI options]
```
...
- RPC mode resets workflow-altering `todo.*`, `task.*`, `advisor.*`, `async.*`, and `bash.autoBackground.*` settings to their built-in defaults instead of inheriting user overrides.
