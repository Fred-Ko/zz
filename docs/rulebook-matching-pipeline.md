...
- project: `<cwd>/.zz/rules/*.{md,mdc}` when the cwd `.zz` directory exists
- user: `~/.zz/agent/rules/*.{md,mdc}`
- sticky user rule: `~/.zz/agent/RULES.md`
- sticky project rule: nearest ancestor `.zz/RULES.md` while walking from cwd toward the repository root
...
- `native` appends project `.zz/rules`, user `~/.zz/agent/rules`, user `RULES.md`, then nearest project `RULES.md`.
