...
```text
~/.zz/agent/sessions/<dir-encoded>/<timestamp>_<sessionId>.jsonl
```
...
```text
~/.zz/agent/blobs/<sha256>
```

Terminal breadcrumb files are written under:

```text
~/.zz/agent/terminal-sessions/<terminal-id>
```
...
- `listAllSessions(storage)` (a.k.a. `SessionManager.listAll()`) -> sessions across all project scopes under `~/.zz/agent/sessions`
