...
- For instruction files discovered into the agent context (`AGENTS.md`, `.zz/`, etc.), see [Context files](./context-files.md).
...
|---|---|---|---|
| Global | `~/.zz/agent/config.yml` | The main persistent settings file. Always loaded. | `/settings`, `zz config set`, and `zz config reset` write here. |
| Global legacy | `~/.zz/agent/settings.json` | Migrated into `config.yml` once, only when `config.yml` does not yet exist. | Not written after migration; the original is renamed to `settings.json.bak`. |
| Project | `<cwd>/.zz/config.yml` (plus `.zz/settings.json`) | Loaded when the process working directory has a non-empty `.zz/`. | Read-only from settings commands; edit the file by hand. |
| Project legacy | `<cwd>/.zz/settings.json` | Still read; project `config.yml` is merged on top of it. | Not written by settings commands. |
...
`PI_CODING_AGENT_DIR` relocates the `~/.zz/agent` base directory. When it is set, the global `config.yml`, the auth store (`agent.db`), and everything else under the agent directory move with it. Use `zz config path` to print the active agent directory.

Native project settings are intentionally scoped to the process working directory's `.zz/` folder — settings discovery does **not** walk ancestor directories looking for the nearest `.zz/`. Other discovery providers (Claude, Codex, Gemini, Cursor, OpenCode) can also contribute project-level settings from their own files; those are read-only from `zz` settings commands and can be turned off by provider id (see [Provider and source disabling](#provider-and-source-disabling)).
...
Use the interactive `/settings` panel inside a session, or the `zz config` command from a shell. Both operate on the merged effective settings, but every persistent write lands in the **global** file only.

```bash
zz config list                 # all settings with current effective values
zz config list --json          # same, machine-readable
zz config get theme.dark       # one value
zz config get theme.dark --json
zz config set compaction.enabled false
zz config set defaultThinkingLevel medium
zz config reset steeringMode   # restore a key to its schema default
zz config path                 # print the active agent directory
```

For users who want the full first-run animation on normal launches, set `startup.showSplash`:

```bash
zz config set startup.showSplash true
```
...
| Command | Effect |
|---|---|
| `zz config list` | Print every setting grouped by tab, with its current value and type. `--json` emits an object keyed by setting path with `{ value, type, description }`. |
| `zz config get <key>` | Print the effective value of one key. Unknown keys exit non-zero. `--json` emits `{ key, value, type, description }`. |
| `zz config set <key> <value>` | Parse `<value>` against the key's schema type and write it to the global `config.yml`. |
| `zz config reset <key>` | Write the key's schema **default** back to the global config (this persists the default, it does not delete the key). |
| `zz config path` | Print the active agent directory (honors `PI_CODING_AGENT_DIR`). |

`zz config` with no subcommand, or `--help`, prints the help and lists settings. The `--json` flag is accepted by `list`, `get`, `set`, and `reset`.

### Value parsing

`zz config set` parses the value string according to the target key's schema type. The string is trimmed first.
...
`zz config set`, `zz config reset`, `/settings`, and any runtime settings change all write to the global `config.yml` under the active agent directory. They never write to `<cwd>/.zz/config.yml`. To create a project-local override, edit that file directly (see [Project-local config](#project-local-config)). Saves are debounced and re-read the file under a lock, so external edits made while a session is open are preserved.
...
3. **Project settings** — `<cwd>/.zz/settings.json` then `<cwd>/.zz/config.yml` (and contributions from other discovery providers at project level).
4. **Global settings** — `~/.zz/agent/config.yml`.
...
| `ZZ_AUTH_BROKER_URL` | `auth.broker.url` | Env value takes precedence over config. |
| `ZZ_AUTH_BROKER_TOKEN` | `auth.broker.token` | Env value takes precedence over config. |
...
Create `<repo>/.zz/config.yml` when a repository needs its own settings:
...
```bash
zz --config ./local/ci-settings.yml "check this failure"
zz --config ./base.yml --config ./experiment.yml "try this model"
```
...
# <repo>/.zz/config.yml — inside this repo ONLY groq is disabled
disabledProviders:
  - groq
```
...
Every key below is defined in the settings schema; `zz config list` shows the full set with current values. Defaults and enum values are taken from the schema. Settings that accept an env or flag override are noted; those overrides are process-local and not persisted.
...
A value of `-1` means "use the provider/model default" — `zz` does not send that parameter.
...
| `tools.approval` | record | `{}` | Per-tool policy keyed by tool name; each value is `allow`, `deny`, or `prompt`. e.g. `zz config set tools.approval '{"bash":"prompt"}'`. |
...
| `auth.broker.url` | string | _(unset)_ | Auth-broker URL. Overridden by `ZZ_AUTH_BROKER_URL`. |
| `auth.broker.token` | string | _(unset)_ | Auth-broker token. Overridden by `ZZ_AUTH_BROKER_TOKEN`. |
...
`zz` migrates older config shapes automatically. None of these require action; they are listed so you know what changes you may see in `config.yml`.

### Startup migration to `config.yml`

When `~/.zz/agent/config.yml` does not exist, startup builds it once from legacy sources, then writes the result:

1. `~/.zz/agent/settings.json` (renamed to `settings.json.bak` after a successful migration).
...
- Start `zz` from the directory that contains `.zz/config.yml`. Settings discovery only checks the current working directory's `.zz/`, not ancestor directories.
- Ensure `.zz/` is non-empty; empty config directories are ignored.
- Confirm the file is valid YAML and its top level is a mapping.
- Run `zz config get <key>` from that directory to see the effective value.
...
### `zz config set` changed the wrong file

`zz config set` and `zz config reset` always write the global `config.yml` under the active agent directory. Run `zz config path` to print it. For project-local settings, edit `<repo>/.zz/config.yml` directly.

### `zz config reset` did not remove my key

`reset` writes the schema **default** value into the global config — it persists the default rather than deleting the key. To stop overriding a project value from global config, delete the key from `~/.zz/agent/config.yml` by hand.
...
### `zz config set <key>` says "Unknown setting"

Keys must match a schema path exactly, with no shorthand. Use `theme.dark`, not `theme`. Run `zz config list` to see every valid key.
