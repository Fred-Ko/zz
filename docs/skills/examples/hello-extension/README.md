...
```
cp -r . ~/.zz/agent/extensions/hello-extension
```

Restart `zz`. You will see the startup notification immediately.
...
```
zz --extension ./hello-extension
```

## Usage

After loading, type `/hello` or `/hello Ada` in the zz prompt. The command sends a visible greeting custom message into the conversation and shows a "Message sent!" notification.
