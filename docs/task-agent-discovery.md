...
`discoverAgents(cwd, home)` (`src/task/discovery.ts`) merges agents from ZZ-native roots and Claude plugin roots before appending bundled definitions. Cross-harness roots such as `.claude/agents`, `.codex/agents`, and `.gemini/agents` are intentionally skipped — their frontmatter schema is not the ZZ task-agent contract (`TASK_AGENT_CONFIG_SOURCE = ".zz"` filters both dir lists).

### Discovery inputs

1. Nearest project `.zz` agents dir from `findAllNearestProjectConfigDirs("agents", cwd)` (filtered to `.zz`; first hit only)
2. User `.zz` agents dir from `getConfigDirs("agents", { project: false })` (filtered to `.zz`; first hit only)
