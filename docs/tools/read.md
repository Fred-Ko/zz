...
- Key collaborators:
  ...
  - `packages/coding-agent/src/internal-urls/router.ts` — resolve `agent://`, `artifact://`, `history://`, `issue://`, `local://`, `mcp://`, `zz://`, `pr://`, `rule://`, `skill://`, and `vault://`.
...
- Registered protocols are outside this file, but the router in `packages/coding-agent/src/internal-urls/router.ts` is built for `agent://`, `artifact://`, `history://`, `issue://`, `local://`, `mcp://`, `zz://`, `pr://`, `rule://`, `skill://`, and `vault://`.
- `#handleInternalUrl()` behavior:
  ...
  - passes `immutable` through to `resolveFileDisplayMode()` so anchors are suppressed for immutable resources such as artifacts, skills, rules, and agent outputs
