...
완료 단계를 직접 수정할 수 없습니다. 새 ID의 대체 단계를 추가하고 `supersedes`로 연결해야 하며, 옛 단계는 삭제되지 않고 lineage에 남습니다. 활성 단계는 superseded·invalidated 단계에 의존할 수 없습니다. 빈 patch는 retry 우회로 간주해 거부합니다.

구조적 변경은 기본 설정에서 기존 승인을 유지하고 바로 실행을 계속합니다. 도구 권한 확대, high-risk 작업, 성공·검증 기준 약화, 완료 결과 무효화 같은 material 변경만 `/zzw approve-plan` 재실행이 필요합니다. `zzworkflow.planPatchApproval: always`이면 모든 patch가 재승인 대상입니다.
