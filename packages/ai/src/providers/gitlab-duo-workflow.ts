 ...
 * keepalive contract ZZ can rely on, so a connection silently going half-open
 ...
 * reports its per-workflow step (graph-recursion) limit. Long ZZ tool-call loops
...
// whose system slot carries ZZ's own authoritative system prompt (no GitLab jinja
...
function createGitLabDuoWorkflowProviderSessionState(): GitLabDuoWorkflowProviderSessionState {
	const state: GitLabDuoWorkflowProviderSessionState = {
		close: () => {
			...
			// PATCH a workflow the server is still running on ZZ would be stranded.
			try {
				state.active?.stop?.();
			} catch {
...
async function runGitLabDuoWorkflow(
	...
	const setupForNamespace = async (
		...
		// The inline `ambient` flow fails server-side without a project, and ZZ has
	...
	try {
		for (let attempt = 0; attempt < 12; attempt++) {
			...
			// A long but healthy ZZ tool-call loop legitimately overruns it; that is
...
// ZZ has no GitLab project of its own, but the inline `ambient` flow fails
		...
		void (async () => {
			...
			while (!settled) {
				...
				if (!handleSocketResult(result, data, pending)) {
					...
					// to ZZ. Clear the pause flag first: the live `onmessage` handler must
...
async function handleGitLabDuoWorkflowSocketMessage(
	...
	if (status === "FAILED" || status === "STOPPED") {
		...
		// RECURSION_LIMIT). A long but healthy ZZ tool-call loop legitimately hits
...
function buildGitLabMcpToolDefinition(tool: Tool): GitLabMcpToolDefinition {
	...
	// the name the model sees, the toolset key it is matched against, and ZZ's own
	// tool docs must all be the same bare name. A prefixed wire name only forced the
	// model to learn `mcp__omp__read` while ZZ docs say `read`, with no upside.
...
// The ZZ system prompt that rides the inline flow's `prompt_template.system` slot.
// DWS wraps it in its own gateway boilerplate, but the slot content is delivered to
// the model verbatim, so ZZ's authoritative rules go here directly — no redirect
...
function renderGitLabDuoWorkflowChatMlToolCall(toolCall: GitLabDuoWorkflowReplayToolCall): string {
	...
	// call). The call id is ZZ-internal wiring the model never reads (call→result pair
...
// The `i` key is ZZ's per-call intent narration (e.g. "Reading kernel smoke body").
...
// Every workflow definition ZZ ships is the inline ambient flow (Path B /
