/**
 * OpenAI Responses HTTP wire-format ↔ ZZ Context bridge for the auth-gateway.
 *
 * Inbound: parses `POST /v1/responses` request bodies into a {@link ParsedRequest}.
 * Outbound: encodes ZZ's {@link AssistantMessage} (and event stream) back into
...
export function parseRequest(body: unknown, headers?: Headers): ParsedRequest {
	...
	// `store` is a stateful-storage hint that ZZ's gateway doesn't honour;
					...
					switch (ev.type) {
						...
						case "toolcall_end": {
							...
							if (cur.customWireName) {
								...
								// Standard JSON tool: arguments object on the ZZ side, the
