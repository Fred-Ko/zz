/**
 * Bun `--preload` shim for the zz dev launcher (`scripts/zz`).
 *
 * The launcher starts Bun from an empty, bunfig-free directory so a foreign
 * project's `bunfig.toml` `preload` cannot run inside the zz CLI: Bun reads
 ...
 */
const launchCwd = process.env.ZZ_LAUNCH_CWD ?? process.env.OMP_LAUNCH_CWD;
if (launchCwd) {
	delete process.env.ZZ_LAUNCH_CWD;
