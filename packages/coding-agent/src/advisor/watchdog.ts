 ...
 * `.zz/<F>` (plus the legacy compatibility directory) for each given filename — and return the readable candidates with
...
export async function collectConfigCandidates(
	...
	// 1. User level: ~/.zz/<F> (or active profile agent dir)
	if (resolvedAgentDir) {
	...
	// 2. Project levels (standalone plus canonical and legacy config dirs): walk up from cwd to repoRoot / home
	let current = cwd;
	while (true) {
		for (const filename of filenames) {
			candidates.add(path.resolve(current, CONFIG_DIR_NAME, filename));
			if (CONFIG_DIR_NAME !== ".omp") candidates.add(path.resolve(current, ".omp", filename));
	...
	for (const candidate of candidates) {
		try {
			...
			const isNativeConfig = baseName === CONFIG_DIR_NAME || baseName === ".omp";
			const ownerDir = isNativeConfig ? path.dirname(parent) : parent;
			const ownerBaseName = ownerDir.split(path.sep).pop() ?? "";
			if (isUser || !ownerBaseName.startsWith(".") || isNativeConfig) {
