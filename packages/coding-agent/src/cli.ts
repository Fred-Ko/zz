...
export async function runCli(argv: string[]): Promise<void> {
	...
	try {
		...
		if (extracted.profile !== undefined) {
			...
			// No explicit --profile: activate any ZZ_PROFILE/OMP_PROFILE/PI_PROFILE inherited
			...
			// ZZ_PROFILE after importing this module (profile aliases/tests). Surfacing
			// validation here turns `ZZ_PROFILE=.. zz --version` into a clean error;
			// calling setProfile keeps every later path helper on the env-selected
			// profile instead of the default agent directory.
			setProfile(resolveProfileEnv(process.env.ZZ_PROFILE ?? process.env.OMP_PROFILE, process.env.PI_PROFILE));
		}
		if (extracted.aliasName !== undefined) {
			const profile = extracted.profile ?? getActiveProfile();
			if (!profile) {
				throw new Error("--alias requires --profile <name> or ZZ_PROFILE");
			}
