...
interface BenchRequestOptions {
	...
	sessionId: string;
	prompt: string;
	/** Native ZZ messages; cache mode splits the stable prefix from the suffix. */
...
export async function runBenchCommand(command: BenchCommandArgs, deps: BenchDependencies = {}): Promise<BenchSummary> {
	...
	if (command.models.length === 0) {
		throw new Error("Pass at least one model selector, e.g. `zz bench opus gpt-5.2`");
	}
		...
		for (const { selector, model, thinking } of targets) {
			...
			if (!preflightKey) {
				const failure: BenchRunFailure = {
					ok: false,
					error: `No credentials for provider "${model.provider}". Run \`zz\` and use /login, or set the provider API key.`,
				};
