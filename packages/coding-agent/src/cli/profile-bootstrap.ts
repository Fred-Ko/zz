 ...
 * doesn't look like another flag. Without this, `zz --system-prompt --profile
 ...
 * `zz --some-ext-flag --profile work` still selects a profile. Known value-less
 * launch flags ({@link VALUELESS_FLAGS}) are exempt so a trailing profile after
 * them also activates (`zz --print --profile work`).
 */
 ...
 * positionals are never stolen (`zz grep --profile <path>` greps for
 * `--profile`; it does not select a profile). `launch` and `acp` are explicit
 * spellings of launch-shaped commands, so `zz launch --profile work` and
 * `zz acp --profile work` still select profile `work`.
 *
...
export function extractProfileFlags(argv: readonly string[]): ProfileBootstrapResult {
	...
	for (let index = 0; index < argv.length; index += 1) {
		...
		// successor only when it is value-like — `zz --bar val --profile work` keeps
		...
		// a trailing global flag still applies (`zz --some-ext-bool --profile work`
		...
		// activates (`zz --print --profile work`).
