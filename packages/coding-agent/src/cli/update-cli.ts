...
const REPO = "Fred-Ko/zz";
const PACKAGE = "@oh-my-pi/pi-coding-agent";
const HOMEBREW_FORMULA = "Fred-Ko/tap/zz";
const MISE_TOOL = "github:Fred-Ko/zz";
/**
...
function isPathInDirectory(filePath: string, directoryPath: string): boolean {
	...
	// PATH-resolved ZZ path can refer to the same directory through different
	...
	// links like Homebrew's `bin/zz -> Cellar/.../bin/zz`; the parent fallback
...
interface UpdateMethodResolutionOptions {
	...
	npmBinDir?: string;
	/**
	 * Whether the resolved ZZ path is a plain file (the standalone binary)
...
/**
 * Run the resolved ZZ binary and check if it reports the expected version.
 */
...
/**
 * Build the bun argv used to globally install a specific ZZ version.
 *
 ...
 *
 * Together these two flags make `zz update` produce exactly the registry
...
export async function runUpdateCommand(opts: { force: boolean; check: boolean }): Promise<void> {
	...
	// Choose update method based on the prioritized ZZ binary in PATH
	try {
