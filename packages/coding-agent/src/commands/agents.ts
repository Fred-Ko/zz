...
export default class Agents extends Command {
	...
	static flags = {
		...
		user: Flags.boolean({ description: "Write to ~/.zz/agent/agents (default)" }),
		project: Flags.boolean({ description: "Write to ./.zz/agents" }),
	};

	static examples = [
		"# Export bundled agents into user config (default)\n  zz agents unpack",
		"# Export bundled agents into project config\n  zz agents unpack --project",
		"# Overwrite existing local agent files\n  zz agents unpack --project --force",
		"# Export into a custom directory\n  zz agents unpack --dir ./tmp/agents --json",
	];

	async run(): Promise<void> {
		const { args, flags } = await this.parse(Agents);
		if (!args.action) {
			renderCommandHelp("zz", "agents", Agents);
			return;
		}
