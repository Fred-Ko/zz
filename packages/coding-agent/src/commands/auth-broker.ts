...
export default class AuthBroker extends Command {
	static description = "Manage the zz auth-broker (credential vault)";
	...
	static examples = [
		"# Boot the broker against the local SQLite store\n  zz auth-broker serve",
		"# Boot on a non-default port\n  zz auth-broker serve --bind=127.0.0.1:9000",
		"# Print the bearer token\n  zz auth-broker token",
		"# Rotate the bearer token\n  zz auth-broker token --regenerate",
		"# List supported OAuth providers\n  zz auth-broker list",
		"# Local login (run on the broker host)\n  zz auth-broker login anthropic",
		"# Interactive provider selection\n  zz auth-broker login",
		"# Remote login over SSH tunnel\n  zz auth-broker login anthropic --via=user@broker",
		"# Log out of a provider (interactive without provider arg)\n  zz auth-broker logout anthropic",
		"# Import a CLIProxyAPI auth dump\n  zz auth-broker import ~/.cliproxy/auth",
		"# Import a single CLIProxyAPI JSON, overriding the provider mapping\n  zz auth-broker import ~/.cliproxy/auth/claude-foo.json --provider anthropic",
		"# Preview a migration from local store + env vars to the configured broker\n  zz auth-broker migrate --from-local --include-env --dry-run",
		"# Apply the migration\n  zz auth-broker migrate --from-local --include-env",
		"# Health-check the configured remote broker\n  zz auth-broker status",
	];

	async run(): Promise<void> {
		const { args, flags } = await this.parse(AuthBroker);
		if (!args.action) {
			renderCommandHelp("zz", "auth-broker", AuthBroker);
			return;
		}
