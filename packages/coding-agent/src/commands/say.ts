...
export default class Say extends Command {
	...
	static examples = [
		'zz say "hello world"',
		"zz say --file notes.md --voice bm_fable",
		'zz say "hello world" --out /tmp/hello.wav',
	];
	...
	#synthesisFailed(model: string): void {
		process.stderr.write(
			chalk.red(
				`error: could not synthesize with local TTS model "${model}". Run \`zz setup speech\` to install it.\n`,
			),
		);
	}
}
