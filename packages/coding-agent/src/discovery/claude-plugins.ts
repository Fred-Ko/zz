...
async function readPluginManifest(root: ClaudePluginRoot): Promise<ClaudePluginManifest | null> {
	for (const manifestDir of [".zz-plugin", ".omp-plugin", ".claude-plugin"]) {
		const raw = await readFile(path.join(root.path, manifestDir, "plugin.json"));
 ...
 *
 * `.zz-plugin/plugin.json` takes precedence, followed by the compatibility
 * `.omp-plugin/plugin.json` and then `.claude-plugin/plugin.json`.
...
async function loadMCPServers(ctx: LoadContext): Promise<LoadResult<MCPServer>> {
	...
	for (const root of roots) {
		...
		if (resolved.inlineServers) {
			...
			//   nested: { "mcpServers": { name: cfg, ... } }   (ZZ/Claude Code project shape)
