 ...
 * SSH JSON Provider
 *
 * Discovers SSH hosts from managed ZZ config paths and legacy root ssh.json files.
...
registerProvider(sshCapability.id, {
	...
	description: "Load SSH hosts from managed zz paths and legacy ssh.json/.ssh.json files",
	priority: 5,
	load,
});
