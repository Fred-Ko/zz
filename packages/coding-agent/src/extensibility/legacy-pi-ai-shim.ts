...
/** Clamp a historical Pi thinking level against ZZ's model metadata. */
 ...
 * were authored against rather than ZZ's own `Error`-based classifiers.
 */
 ...
 * `@earendil-works/pi-ai` exposed from its package root but ZZ's
 ...
 * static named-export check (e.g. `zz plugin install pi-blackhole`).
 *
 ...
 * `@oh-my-pi/pi-ai/error`) and the JSON-repair helpers that ZZ relocated to
 * `@oh-my-pi/pi-utils`. Upstream root symbols with no ZZ equivalent are
