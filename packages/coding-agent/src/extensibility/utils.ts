	...
	constructor(
		...
		super(
			`Module called ${alias}(${code === undefined ? "" : String(code)}) during guarded extension/hook loading; ` +
				`ZZ extension/hook modules must not terminate the host process.`,
		);
 ...
 * from user directories (including Claude Code's `~/.claude/tools`, which ZZ
 ...
 *   ZZ during startup with no error surface, since `try/catch` cannot
