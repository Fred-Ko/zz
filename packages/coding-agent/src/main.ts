...
// re-apply at startup so embedders inherit ZZ's neutral defaults instead of
...
async function runInteractiveMode(
	...
	// `zz join <link>`: dispatch through the same builtin path as a typed
...
function discoverSystemPromptFile(): string | undefined {
	// Check project-local first (.zz/SYSTEM.md, then legacy .omp/.pi locations)
...
export async function runRootCommand(
	...
	if (mode === "acp") {
		...
		// Fail fast on stale/typo flags (e.g. `zz --list-models`) now that we
