...
/** Credential-id prefix for ZZ-managed MCP OAuth credentials keyed by profile and server URL. */
...
/**
 * Deterministic credential id for an MCP server URL scoped to a ZZ profile.
 *
...
/** Whether a credential id was minted by ZZ's MCP OAuth flows (either era). */
 ...
 *
 * Plane is stricter for ZZ-synthesized fallback resources (e.g. using the
...
export class MCPOAuthFlow extends OAuthCallbackFlow {
	...
	async generateAuthUrl(state: string, redirectUri: string): Promise<{ url: string; instructions?: string }> {
		...
		if (existingResource) {
			...
			// provider-authored, not ZZ's server-URL fallback. Preserve same-host
