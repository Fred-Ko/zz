...
import { gradientEscape, gradientLogo, type ShineConfig, ZZ_LOGO } from "../../components/welcome";
...
const LARGE_LOGO = ZZ_LOGO.flatMap(line => {
	let wide = "";
...
function renderCompactSplash(width: number, height: number, phase: number, shine: ShineConfig): string[] {
	const art = height >= 14 ? LARGE_LOGO : ZZ_LOGO;
