...
- DB·Kafka·Docker daemon·생성 파일처럼 현재 단계의 승인 범위 안에서 충족할 수 있는 실행 전제가 빠졌다면 `classification=missing-precondition`입니다. work 단계는 같은 단계에서 바로 준비합니다. validation 단계는 Runtime이 `satisfy-approved-precondition` 복구 상태를 열며, 현재 단계의 tool/target 범위에서 전제를 준비·진단한 뒤 성공한 operation evidence로 `status=progress`, `classification=matched`를 보고해야 exact validator를 다시 실행할 수 있습니다. 어느 경우에도 Plan을 수정하거나 재승인을 요청하지 않습니다.
