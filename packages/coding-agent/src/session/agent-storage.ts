...
/** Row shape read from a `zz stats` messages table during backfill. */
...
`);
	 ...
	 * `zz stats` database (`~/.zz/stats.db`) into model_perf. Fire-and-forget:
	 ...
	 * A missing stats.db leaves the marker unset so a later `zz stats` run
	...
	/**
	 * Imports recent measurable request rows from a `zz stats` database
