/**
 * Managed-skills primitives for ZZ's independent skill subsystem.
 *
 * Managed skills are generated/enhanced `SKILL.md` files kept in an isolated
 * directory (`~/.zz/agent/managed-skills`) separate from user-authored skills
 * (`~/.zz/agent/skills`). They are discovered and
 * surfaced like normal skills, but every write here is confined to
 * `getManagedSkillsDir()` — management can never touch authored skills.
 */
...
export const MANAGED_SKILLS_PROVIDER_ID = "zz-managed";
...
/** Resolve the isolated managed-skills directory (`~/.zz/agent/managed-skills`). */
