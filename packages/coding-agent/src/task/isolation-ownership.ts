/**
 * Ownership marker for task-isolation sandboxes under `~/.zz/wt/`.
 *
 * Each isolation base dir (`ensureIsolation` in {@link ./worktree}) holds a
 * compact `m` mount plus this marker file naming the ZZ process that created
 * it. `zz worktree clear` consults the marker so it can distinguish a live
...
export const ISOLATION_OWNER_FILE = ".zz-isolation-owner.json";

/** Recorded owner of a task-isolation sandbox. */
export interface IsolationOwner {
	/** PID of the ZZ process that created and owns the sandbox. */
	pid: number;
 ...
 * `zz worktree clear` never sees an owner-less sandbox mid-creation.
 */
...
/**
 * Whether a live ZZ process still owns the sandbox at `baseDir`.
 *
