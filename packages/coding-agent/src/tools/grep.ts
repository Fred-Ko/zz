...
const DOCS_ROOT_URL_RE = /^(?:zz|omp):\/\/(?:\/?|docs\/?)$/i;
...
async function expandVirtualInternalResource(
	...
	if (DOCS_ROOT_URL_RE.test(rawPath)) {
		const scheme = rawPath.toLowerCase().startsWith("omp:") ? "omp" : "zz";
		const completions = await internalRouter.complete(scheme, "");
		if (completions && completions.length > 0) {
			...
			for (const completion of completions) {
				...
				const docUrl = `${scheme}://${completion.value}`;
