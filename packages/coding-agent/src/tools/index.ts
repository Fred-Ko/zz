...
export async function createTools(session: ToolSession, toolNames?: string[]): Promise<Tool[]> {
	...
	// Top-level sessions keep ZZW control tools registered but dormant until a
	// ZZWorkflow-controlled goal activates them. The canonical registry must own
	// these instances so mid-session activation and xd:// presentation never need
	// a parallel registry or a second factory path.
	if (!restrictToolNames && (session.taskDepth ?? 0) === 0) {
		for (const name of ZZWORKFLOW_TOOL_NAMES) {
			if (toolRegistry.has(name)) continue;
			const tool = await logger.time(`createTools:${name}:dormant`, BUILTIN_TOOLS[name], session);
			if (!tool) continue;
