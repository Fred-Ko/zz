...
describe("discoverAdvisorConfigs", () => {
	...
	beforeEach(async () => {
		...
		// Empty agent dir so the user-level search path can't pick up a real ~/.zz/WATCHDOG.yml.
...
describe("WATCHDOG.yml file round-trip", () => {
	...
	it("resolves project and user scope paths", () => {
		expect(advisorConfigFilePath("project", { projectDir: "/repo", agentDir: "/home/.zz" })).toBe(
			path.join("/repo", "WATCHDOG.yml"),
		);
		expect(advisorConfigFilePath("user", { projectDir: "/repo", agentDir: "/home/.zz" })).toBe(
			path.join("/home/.zz", "WATCHDOG.yml"),
		);
	});
});
