...
/**
 * Regression for #6761: `zz worktree clear` (no `--all`) must delete only
 * task-isolation sandboxes whose owner process is gone. A sandbox owned by a
 * live ZZ process holds a running subagent's uncaptured work and must survive.
 */
describe("worktree clear task-isolation ownership", () => {
	...
	beforeEach(async () => {
		base = await fs.mkdtemp(path.join(os.tmpdir(), "zz-wt-clear-"));
