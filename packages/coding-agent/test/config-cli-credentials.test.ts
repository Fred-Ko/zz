...
describe("credential settings", () => {
	...
	it("classifies UI-visible credentials through the same marker", () => {
		...
		for (const path of ["knowledge.hindsight.apiToken"] as const) {
			expect(isCredential(path)).toBe(true);
		}
	});
...
describe("config list output", () => {
	...
	beforeEach(() => {
		...
		agentDir = TempDir.createSync("@zz-config-credentials-");
	...
	it("leaves the Hindsight server URL readable", async () => {
		...
		await runConfigCommand({ action: "set", key: "knowledge.hindsight.apiUrl", value: url, flags: { json: true } });
		await runConfigCommand({
			action: "set",
			key: "knowledge.hindsight.apiToken",
			value: SECRET,
			flags: { json: true },
		});
		expect(isCredential("knowledge.hindsight.apiUrl")).toBe(false);

		const output = await humanList();
		expect(output).toContain(`knowledge.hindsight.apiUrl = ${url}`);
		expect(output).toContain("knowledge.hindsight.apiToken = ********");
		expect(output).not.toContain(SECRET);

		const { parsed } = await jsonList();
		expect(parsed["knowledge.hindsight.apiUrl"]).toMatchObject({ value: url });
		expect(parsed["knowledge.hindsight.apiToken"]).toMatchObject({ redacted: true });
	});
});
