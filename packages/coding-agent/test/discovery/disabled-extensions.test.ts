...
describe("disabledExtensions runtime filtering", () => {
	...
	beforeEach(async () => {
		...
		setAgentDir(path.join(tempHomeDir, ".zz", "agent"));
		tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "omp-disabled-ext-"));
		await fs.mkdir(path.join(tempDir, ".zz"), { recursive: true });
		await fs.writeFile(path.join(tempDir, ".zz", "AGENTS.md"), "# project instructions\n");
