...
describe("guided goal setup", () => {
	...
	it("uses the ZZW interview contract for /zzw-guided-goal without changing ordinary guided goal", async () => {
		const harness = await createHarness();
		try {
			const promptSpy = vi.spyOn(harness.session, "prompt").mockResolvedValue(true);

			await harness.mode.handleZZWorkflowGuidedGoalCommand("migrate the persistence layer");

			expect(promptSpy).toHaveBeenCalledTimes(1);
			const [text, promptOptions] = promptSpy.mock.calls[0]!;
			expect(promptOptions).toEqual({ synthetic: true });
			expect(text).toContain("migrate the persistence layer");
			expect(text).toContain('controller: "zzworkflow"');
			expect(text).toContain("Task Contract and Plan DAG");
			expect(harness.session.getEnabledToolNames()).toContain("goal");
		} finally {
			await harness.cleanup();
		}
	...
	it("refuses to start while goal mode is disabled, active, or paused", async () => {
		...
		try {
			...
			expect(promptSpy).not.toHaveBeenCalled();
			expect(warning).toHaveBeenCalledWith("목표 모드가 비활성화되어 있습니다. 설정에서 goal.enabled를 켜세요.");
		...
		try {
			...
			await harness.mode.handleGuidedGoalCommand("ship it");
			expect(promptSpy).not.toHaveBeenCalled();
			expect(warning).toHaveBeenCalledWith("먼저 현재 목표를 재개하거나 삭제한 뒤 새 목표를 설정하세요.");
		} finally {
			await harness.cleanup();
		}
	});
	...
	it("goal tool create assigns the ZZWorkflow controller when explicitly requested", async () => {
		const harness = await createHarness();
		try {
			await harness.goalTool.execute("call-zzw", {
				op: "create",
