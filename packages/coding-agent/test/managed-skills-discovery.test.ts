...
import { getManagedSkillsDir } from "@oh-my-pi/pi-coding-agent/skills/managed";
...
describe("managed-skills discovery", () => {
	...
	beforeEach(async () => {
		...
		tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "zz-managed-disco-home-"));
		// cwd MUST live under the fake home so loadSkills' ancestor walk is bounded
		// and cannot pick up ambient /tmp/.zz or /.zz fixtures (full-suite-safe).
		tempCwd = path.join(tempHome, "work");
		await fs.mkdir(tempCwd, { recursive: true });
		spyOn(os, "homedir").mockReturnValue(tempHome);
		setAgentDir(path.join(tempHome, ".zz", "agent"));
	...
	it("surfaces a managed skill tagged with the zz-managed provider", async () => {
		await writeSkill(managedDir, "foo", "A managed skill.");
		const { skills } = await loadSkills({ cwd: tempCwd });
		const foo = skills.find(s => s.name === "foo");
		expect(foo).toBeDefined();
		expect(foo?.source).toBe("zz-managed:user");
	});

	it("exposes the bundled knowledge operator only when ZZ Knowledge is active", async () => {
		const { skills } = await loadSkills({
			...
			enableBundledKnowledge: true,
		});
		const operator = skills.find(skill => skill.name === "knowledge-operator");

		expect(operator?.source).toBe("zz-bundled:user");
		expect(operator?.content).toContain("Call `knowledge_recall` only when prior knowledge can change");
		expect(operator?.content).toContain("Mental models never refresh automatically.");

		const inactive = await loadSkills({
			cwd: tempCwd,
			enableCodexUser: false,
			enableClaudeUser: false,
			enableClaudeProject: false,
		...
		expect(inactive.skills.some(skill => skill.name === "knowledge-operator")).toBe(false);
		expect(
			inactive.skills
				.filter(skill => skill.source === "zzw-bundled:user")
				.map(skill => skill.name)
				.sort(),
		).toEqual(["zzw-plan-evolution", "zzw-reconciliation", "zzw-verification"]);
	});
	...
	it("lets an authored skill win a name collision and drops the managed one", async () => {
		...
		expect(skills.some(s => s.name === "bar" && s.source === "zz-managed:user")).toBe(false);
	});
	...
	it("lets an authored skill from a NON-native provider win over a managed skill", async () => {
		...
		expect(skills.some(s => s.name === "baz" && s.source === "zz-managed:user")).toBe(false);
	});
	...
	it("keeps a managed skill visible even when a disabled provider has the same name", async () => {
		...
		expect(dises[0]?.source).toBe("zz-managed:user");
	});
	...
	it("keeps managed skills dead-last behind an enabled authored fallback hidden by a disabled duplicate (#4648)", async () => {
		...
		expect(skills.some(s => s.name === "shadowed" && s.source === "zz-managed:user")).toBe(false);
	});
	...
	it("skips a managed skill whose on-disk frontmatter name is unsafe", async () => {
		...
		expect(skills.some(s => s.source === "zz-managed:user")).toBe(false);
	});
