...
function mockPluginManagerPaths(root: string) {
	return [
		...
		spyOn(piUtils, "getPluginsLockfile").mockReturnValue(path.join(root, "zz-plugins.lock.json")),
...
describe("MarketplaceManager", () => {
	...
	it("installPlugin → plugin in cache + in registry", async () => {
		...
		const runtimeConfig = await Bun.file(path.join(ctx.tmpDir, "zz-plugins.lock.json")).json();
	...
	it("installPlugin exposes marketplace package to the runtime loader", async () => {
		...
		try {
			const pluginsDir = path.join(tmpHome, ".zz", "plugins");
			const manager = new MarketplaceManager({
				marketplacesRegistryPath: path.join(tmpHome, ".zz", "marketplaces.json"),
	...
	it("installPlugin with scope:project exposes the marketplace package to the runtime loader", async () => {
		...
		try {
			const userPluginsDir = path.join(tmpHome, ".zz", "plugins");
			const projectPluginsDir = path.join(projectAnchor, ".zz", "plugins");
			...
			const projectLock = await Bun.file(path.join(projectPluginsDir, "zz-plugins.lock.json")).json();
			...
			expect(fs.existsSync(path.join(userPluginsDir, "zz-plugins.lock.json"))).toBe(false);
		} finally {
			fs.rmSync(tmpHome, { recursive: true, force: true });
	...
	it("setPluginEnabled → persisted in registry", async () => {
		...
		let runtimeConfig = await Bun.file(path.join(ctx.tmpDir, "zz-plugins.lock.json")).json();
		expect(runtimeConfig.plugins["hello-plugin"].enabled).toBe(false);
		...
		runtimeConfig = await Bun.file(path.join(ctx.tmpDir, "zz-plugins.lock.json")).json();
