...
const ORIGINAL_TIMEOUT = process.env.ZZ_MCP_TIMEOUT_MS;
const ORIGINAL_LEGACY_TIMEOUT = process.env.OMP_MCP_TIMEOUT_MS;

afterEach(() => {
	if (ORIGINAL_TIMEOUT === undefined) {
		delete process.env.ZZ_MCP_TIMEOUT_MS;
	} else {
		process.env.ZZ_MCP_TIMEOUT_MS = ORIGINAL_TIMEOUT;
	}
	if (ORIGINAL_LEGACY_TIMEOUT === undefined) {
		delete process.env.OMP_MCP_TIMEOUT_MS;
	} else {
		process.env.OMP_MCP_TIMEOUT_MS = ORIGINAL_LEGACY_TIMEOUT;
	}
});
...
describe("MCP timeout configuration", () => {
	...
	test("allows the env override to disable MCP client-side timeouts", () => {
		process.env.ZZ_MCP_TIMEOUT_MS = "0";
	...
	test("rejects non-numeric env values and falls back to the default", () => {
		process.env.ZZ_MCP_TIMEOUT_MS = "not-a-number";
		const warn = spyOn(logger, "warn").mockImplementation(() => {});
