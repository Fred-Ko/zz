...
describe("plugin manifest path resolution", () => {
	it("resolves a directory tools entry to its index, not the zz.extensions modules", () => {
		const dir = fs.mkdtempSync(path.join(os.tmpdir(), "zz-manifest-paths-"));
		try {
			// The package declares both extensions and a directory-based tool entry.
			// `zz.extensions` and the sub-extension scan are extensions-specific and
			...
			fs.writeFileSync(
				...
				JSON.stringify({ name: "fixture-plugin", version: "1.0.0", zz: { extensions: ["./ext.ts"], tools: "." } }),
			);
