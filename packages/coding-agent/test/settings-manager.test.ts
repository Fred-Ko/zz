...
describe("Settings", () => {
	...
	describe("config file failure safety", () => {
		...
		it("backs up a corrupted project config and retains the pending project role for retry", async () => {
			await writeSettings({});
			const projectConfigPath = path.join(projectDir, ".zz", "config.yml");
			await Bun.write(
				projectConfigPath,
		...
		it("observes both failures when global and project configs are malformed", async () => {
			...
			await Promise.all([
				...
				Bun.write(path.join(projectDir, ".zz", "config.yml"), malformed),
			]);
			...
			try {
				...
				expect(
					fs.readdirSync(path.join(projectDir, ".zz")).some(name => name.startsWith("config.yml.broken-")),
				).toBe(true);
			} finally {
	...
	describe("migrations", () => {
		...
		it("migrates the quoted dotted reminders key", async () => {
		...
		it("lets the explicit reminders key win over its legacy nested value", async () => {
			await writeSettings({
		...
		it("preserves the reminders parent boolean alongside its legacy leaf key", async () => {
		...
		it("drops retired automatic-QA and shared-coordinator settings while migrating local ZZW settings", async () => {
			await writeSettings({
				dev: {
					...
					autoqaPush: { endpoint: "https://qa.example.test", token: "secret" },
				},
				"dev.autoqa": true,
			...
			// Touch an unrelated key so the cleaned tree is written back.
			settings.set("display.showTokenUsage", true);
			await settings.flush();
			...
			expect(onDisk.dev).toBeUndefined();
