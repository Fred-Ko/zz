...
function createRuntime(handler: () => Promise<void>) {
	...
	const handleZZWorkflowGuidedGoalCommand = vi.fn(handler);
	const setText = vi.fn();
	return {
		handleGuidedGoalCommand,
		handleZZWorkflowGuidedGoalCommand,
		setText,
		runtime: {
			ctx: {
...
describe("/zzw-guided-goal slash command", () => {
	it("clears the slash draft before the controlled interview turn resolves", async () => {
		const { promise, resolve } = Promise.withResolvers<void>();
		const harness = createRuntime(() => promise);

		const dispatched = executeBuiltinSlashCommand("/zzw-guided-goal migrate persistence", harness.runtime);
		...
		expect(await dispatched).toBe(true);
		expect(harness.setText).not.toHaveBeenCalled();
		expect(harness.handleZZWorkflowGuidedGoalCommand).toHaveBeenCalledWith("migrate persistence");
	});
});
