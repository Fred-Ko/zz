...
async function writeOmpPluginAgent(home: string): Promise<void> {
	const userPluginsRoot = path.join(home, ".zz", "plugins");
...
describe("discoverAgents", () => {
	...
	test("loads OMP agents but skips Claude Code custom agents", async () => {
		await fs.mkdir(path.join(projectDir, ".zz", "agents"), { recursive: true });
		await fs.writeFile(path.join(projectDir, ".zz", "agents", "omp-test-agent.md"), OMP_AGENT_MD);
		...
		expect(projectAgentsDir).toBe(path.join(projectDir, ".zz", "agents"));
	});

	test("loads agents from ZZ npm plugins under <home>/.zz/plugins/node_modules", async () => {
	...
	test("CLI extension agents win over project `extensions:` settings on dedup", async () => {
		...
		await fs.mkdir(path.join(projectDir, ".zz"), { recursive: true });
		await fs.writeFile(path.join(projectDir, ".zz", "settings.json"), JSON.stringify({ extensions: [projectExt] }));
