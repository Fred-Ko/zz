...
describe("update-cli install target detection", () => {
	...
	it("uses binary update when a plain file in the npm global bin dir is the standalone binary, not an npm symlink", () => {
		...
		const method = resolveUpdateMethodForTest("/home/u/.local/bin/zz", undefined, {
	...
	it("uses binary update when a plain file in the bun global bin dir is the standalone binary", () => {
		const method = resolveUpdateMethodForTest("/home/u/.local/bin/zz", "/home/u/.local/bin", {
			ompIsRegularFile: true,
		});
	...
	it("keeps bun update for regular-file entries in the bun global bin dir on Windows, where bun writes .exe shims", () => {
		...
		try {
			const method = resolveUpdateMethodForTest("C:/Users/test/.bun/bin/zz.exe", "C:/Users/test/.bun/bin", {
				ompIsRegularFile: true,
			});
...
describe("update-cli package manager commands", () => {
	it("targets the Homebrew tap formula and switches to reinstall for forced updates", () => {
		expect(buildHomebrewUpdateArgs(false)).toEqual(["upgrade", "Fred-Ko/tap/zz"]);
		expect(buildHomebrewUpdateArgs(true)).toEqual(["reinstall", "Fred-Ko/tap/zz"]);
	});

	it("targets the mise GitHub backend tool and force-reinstalls the checked version when requested", () => {
		expect(buildMiseUpgradeArgs()).toEqual(["upgrade", "github:Fred-Ko/zz", "--bump"]);
		expect(buildMiseForceInstallArgs("15.10.5")).toEqual(["install", "--force", "github:Fred-Ko/zz@15.10.5"]);
	});
...
describe("update-cli release binary integrity", () => {
	const tag = "v17.1.2";
	const binaryName = "zz-linux-x64";
	const url = `https://github.com/Fred-Ko/zz/releases/download/${tag}/${binaryName}`;
	...
	it("rejects release metadata that does not identify one exact stable asset", () => {
		...
		expect(() =>
			resolveReleaseBinaryAsset(
				releaseAsset({ browser_download_url: "https://example.com/zz-linux-x64" }),
				tag,
				binaryName,
			),
