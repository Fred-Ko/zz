...
1. **Local zz, not npm.** By default the runner bind-mounts the repo
   read-only into each task container (`--install source`) and runs zz
   ...
   `/opt/zz/bin` — so trial setup needs zero outbound network. Alternatives:
   `--install local` (pack a tarball per run) or `--binary` (prebuilt
   `dist/zz-linux-*` self-contained binaries).
...
| `--agent-arg <arg>` | — | Extra arg forwarded verbatim to the in-container zz CLI (repeatable) |
| `--env <KEY[=VALUE]>` | — | Forward env into the zz container (repeatable); `KEY` alone forwards the host value |
| `--binary <path>` | — | Prebuilt zz binary (repeat for arm64+x64) |
...
arc; ~$0.001 per report). API keys resolve through zz's auth storage.
