/**
 * Set the OS-visible process name (`/proc/self/comm`) so `zz` shows up as
 * `zz` — not `bun` — in `ps`, `pgrep`, `killall`, `top`, `htop`, and systemd.
 *
 ...
 * `bun` and process-name-based tooling can't target ZZ (and `pkill bun` becomes
