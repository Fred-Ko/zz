...
describe("document conversion cache directory", () => {
	...
	it("uses XDG_CACHE_HOME for the default agent dir when $XDG_CACHE_HOME/zz exists", async () => {
		...
		expect(getDocumentConversionCacheDir()).toBe(
			path.join(process.env.XDG_CACHE_HOME, "zz", "cache", "document-conversions"),
		);
	});
