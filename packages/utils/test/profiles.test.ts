...
describe("profile directories", () => {
	...
	it("keeps XDG-backed named profile state under profile-specific roots", async () => {
		...
		await fs.mkdir(path.join(process.env.XDG_DATA_HOME, "zz", "profiles", "work"), { recursive: true });
		await fs.mkdir(path.join(process.env.XDG_STATE_HOME, "zz", "profiles", "work"), { recursive: true });
		await fs.mkdir(path.join(process.env.XDG_CACHE_HOME, "zz", "profiles", "work"), { recursive: true });

		setProfile("work");

		expect(getAgentDbPath()).toBe(path.join(process.env.XDG_DATA_HOME, "zz", "profiles", "work", "agent.db"));
		expect(getSessionsDir()).toBe(path.join(process.env.XDG_DATA_HOME, "zz", "profiles", "work", "sessions"));
		expect(getPythonGatewayDir()).toBe(
			path.join(process.env.XDG_STATE_HOME, "zz", "profiles", "work", "python-gateway"),
		);
	});
	...
	it("does not silently switch a named profile to XDG once the base app dir appears", async () => {
		...
		await fs.mkdir(path.join(process.env.XDG_DATA_HOME, "zz"), { recursive: true });
		await fs.mkdir(path.join(process.env.XDG_STATE_HOME, "zz"), { recursive: true });
		await fs.mkdir(path.join(process.env.XDG_CACHE_HOME, "zz"), { recursive: true });
	...
	it("rejects path-like profile names", () => {
		expect(() => setProfile("../work")).toThrow("Invalid ZZ profile");
		expect(() => setProfile("work/team")).toThrow("Invalid ZZ profile");
	});
...
describe("profile env + name validation", () => {
	...
	it("rejects uppercase profile names so isolation is filesystem-independent", () => {
		...
		expect(() => normalizeProfileName("WORK")).toThrow("Invalid ZZ profile");
		expect(() => normalizeProfileName("Work")).toThrow("Invalid ZZ profile");
...
describe("dirs module import behavior", () => {
	...
	it("honors XDG dir keys from a profile .env applied after the resolver froze", async () => {
		...
		try {
			...
			// Named profiles only adopt XDG when their own XDG path already exists.
			const xdgProfileRoot = path.join(xdgStateRoot, "zz", "profiles", "work");
