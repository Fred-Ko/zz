#!/usr/bin/env bun
//
// Render the Homebrew formula for `zz` from a published GitHub release and write
// it to a tap checkout. The release publishes per-platform bare binaries
// (zz-<platform>-<arch>); this reads their sha256 digests straight from the
// release metadata so the formula never drifts from the shipped assets.
//
// Usage:
//   bun scripts/ci-update-brew-formula.ts <tag> --out <path/to/Formula/zz.rb>
...
export function renderFormula(version: string, sums: Record<string, string>): string {
	...
	// which nests the file outside the staging CWD; `Dir["zz-*"].first` then
	// returns `nil` and `bin.install nil => "zz"` raises.
	//
	...
	// not touch the real `/Users/<user>/.zz` (denied by Homebrew's sandbox
	// profile, which would otherwise fail the popen).
	return `class Zz < Formula
  desc "${DESC}"
  homepage "${HOMEPAGE}"
  version "${version}"
  license "MIT"
  ...
  on_macos do
    on_arm do
      url "https://github.com/${REPO}/releases/download/v#{version}/zz-darwin-arm64",
          using: :nounzip
      sha256 "${sums["zz-darwin-arm64"]}"
    end
    on_intel do
      url "https://github.com/${REPO}/releases/download/v#{version}/zz-darwin-x64",
          using: :nounzip
      sha256 "${sums["zz-darwin-x64"]}"
    end
  end

  on_linux do
    on_arm do
      url "https://github.com/${REPO}/releases/download/v#{version}/zz-linux-arm64",
          using: :nounzip
      sha256 "${sums["zz-linux-arm64"]}"
    end
    on_intel do
      url "https://github.com/${REPO}/releases/download/v#{version}/zz-linux-x64",
          using: :nounzip
      sha256 "${sums["zz-linux-x64"]}"
    end
  end

  def install
    bin.install Dir["zz-*"].first => "zz"
    (bin/"zz").chmod 0555
    with_env(HOME: buildpath) do
      generate_completions_from_executable(bin/"zz", "completions", shells: [:bash, :zsh, :fish])
    end
  end

  test do
    assert_match version.to_s, shell_output("#{bin}/zz --version")
  end
end
`;
}
...
async function main(): Promise<void> {
	...
	const targets = ["zz-darwin-arm64", "zz-darwin-x64", "zz-linux-arm64", "zz-linux-x64"];
