...
if [ "${ZZ_INSTALL_TEST_SKIP_NATIVE_BUILD:-${OMP_INSTALL_TEST_SKIP_NATIVE_BUILD:-0}}" != "1" ]; then
...
for pkg in utils wire hashline catalog ai snapcompact agent tui stats collab-web; do
   (
...
(
   ...
   bun add "$utils_tgz" "$wire_tgz" "$natives_tgz" "$hashline_tgz" "$catalog_tgz" "$ai_tgz" "$snapcompact_tgz" "$agent_tgz" "$tui_tgz" "$stats_tgz" "$coding_agent_tgz" "$collab_web_tgz"
