...
function Configure-BashShell {
    try {
        $settingsDir = Join-Path $env:USERPROFILE ".zz\agent"
...
function Install-ViaBun {
    ...
    if ($Ref) {
        ...
        $tmpRoot = Join-Path ([System.IO.Path]::GetTempPath()) ("zz-install-" + [System.Guid]::NewGuid().ToString("N"))
    ...
    Write-Host ""
    Write-Host "✓ Installed zz via bun" -ForegroundColor Green

    Configure-BashShell

    Write-Host "Run 'zz' to get started!"
}
...
function Install-Binary {
    ...
    $OutPath = Join-Path $InstallDir "zz.exe"
    Invoke-WebRequest -Uri $BinaryUrl -OutFile $OutPath -TimeoutSec 900

    Write-Host ""
    Write-Host "✓ Installed zz to $OutPath" -ForegroundColor Green
    ...
    if ($needsRestart) {
        Write-Host "Restart your terminal, then run 'zz' to get started!"
    } else {
        Write-Host "Run 'zz' to get started!"
    }
}
