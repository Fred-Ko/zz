...
const linkScript = path.join(repoRoot, "scripts", "link-zz.sh");
const targetWrapper = path.join(repoRoot, "packages", "coding-agent", "scripts", "zz");
const tempDirs: string[] = [];

function makeTempDir() {
	const dir = fs.mkdtempSync(path.join(os.tmpdir(), "zz-link-"));
	tempDirs.push(dir);
	return dir;
}
...
describe("scripts/link-zz.sh", () => {
	it("falls back to BUN_INSTALL/bin when bun cannot resolve the global bin", () => {
		...
		expect(fs.readlinkSync(path.join(bunInstall, "bin", "zz"))).toBe(targetWrapper);
	});
	...
	it("uses bun pm -g bin when it succeeds", () => {
		...
		expect(result.status).toBe(0);
		expect(result.stderr).toBe("");
		expect(fs.readlinkSync(path.join(globalBin, "zz"))).toBe(targetWrapper);
		expect(fs.existsSync(path.join(dir, "unused-bun-install", "bin", "zz"))).toBe(false);
	});
});
