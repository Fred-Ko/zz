...
def resolve_zz_bin(raw: str | None) -> str:
    if raw:
        return raw
    found = shutil.which("zz")
    if not found:
        raise SystemExit("Could not find `zz` on PATH. Set --zz-bin or ZZ_BIN.")
    return found
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run OpenRouter fixture evaluations through ZZ RPC mode."
    )
    parser.add_argument("--zz-bin", default=os.environ.get("ZZ_BIN"))
...
async def run_all(args: argparse.Namespace) -> int:
    zz_bin = resolve_zz_bin(args.zz_bin)
