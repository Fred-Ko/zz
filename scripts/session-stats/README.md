...
(`~/.zz/agent/sessions/`). SQLite-backed; data is synced once into the same
`~/.zz/stats.db` that `packages/stats` uses, then queried by short Python
scripts.
...
scripts/session-stats/
  sync.py          # walks ~/.zz/agent/sessions/ and populates ss_* tables
...
credentials resolved through ZZ's auth storage — stored key, OAuth, or env
...
Verdicts are cached in `~/.zz/stats-audit-cache.json` (keyed by session id +
