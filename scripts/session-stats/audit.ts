#!/usr/bin/env bun
/**
 * Token-usage audit over the local ZZ session corpus (~/.zz/agent/sessions/).
 *
 ...
 *
 * Auth: resolves an API key for the classifier provider through ZZ's auth
 * storage (~/.zz/agent/agent.db: stored key, OAuth, or env var fallback).
 */
...
const SESSIONS_ROOT = path.join(os.homedir(), ".zz", "agent", "sessions");
const DEFAULT_MODEL = "anthropic/claude-sonnet-4-6";
const CACHE_PATH = path.join(os.homedir(), ".zz", "stats-audit-cache.json");
...
function parseCli(argv: string[]): CliOptions {
	...
	if (values.help) {
		console.log(
			`session audit — token usage analysis over ~/.zz/agent/sessions\n\n` +
				...
				`  --no-cache               skip the verdict cache (~/.zz/stats-audit-cache.json)\n` +
